...
NpassosMax = 10 # Numero maximo de passos em cada experimento
step = 20

Nshow = 5  # Numero de experimentos a serem mostrados na animacao
...
for iexp in range(Nshow):
   t = turtle.Turtle()
   t.speed(2)  # Speeds from 1 to 10 enforce increasingly faster animation of line drawing and turtle turning. speed = 0 means that no animation
   t.shape("circle")
...
def oneStep():
      ...
      theta = random.uniform(0,2*np.pi)  # Metodo simples, didatico para sortear a direcao
    ...
    #      dx=-1; dy=0 #  Oeste
      ...
      x[iexp]+=np.cos(theta)*step
      y[iexp]+=np.sin(theta)*step
      z[iexp]+=sqrt(x[iexp]**2+y[iexp]**2)
      #if iexp==0: print(Npassos,iexp,dx,dy,x[iexp],y[iexp])

      if iexp < Nshow: man[iexp].goto(x[iexp],y[iexp],z[iexp])
